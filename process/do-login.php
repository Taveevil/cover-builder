...
if (!empty($rqst['username'])) {
	// They entered something in the email address field
	
	// Grab the data we need
	$login_user = mysql_read_user_by_name($rqst['username']);
	...
	if ($login_user) {
		...
		header('Location: '.$site['url'].'/login.php/?login_error=password');
		die();
	}

	// User's email address wasn't found in the database
	header('Location: '.$site['url'].'/login.php/?login_error=user');
	die();
}


// Kick them out...
header('Location: '.$site['url'].'/login.php/?login_error=blank');
die();
